...
load_seg(
    ...
    const struct x86_emulate_ops *ops)
{
    printf("LOAD_SEG! CTXT %X seg %d sel %ud\n", ctxt, seg, sel);
    if ( (ops->read_segment == NULL) ||
...
x86_emulate(
    ...
    const struct x86_emulate_ops  *ops)
{
    // all of my fixes (fix_eax,fix_ebx,etc.. are all due to corruption somewhere...
    // and now that rc is returning false information .. im sure of it
    // the problem is where... i can either keep padding idiotically... or
    // track it down.. today.. i'll add padding and hope it helps...
    // i'll have to track it down another day when i give a fuck
    
    // *DEF 0days in x86_emulate() that are not found yet!!!
    // check other exploits to see how they force x86_emulate()
    // and itll work on all xen.. maybe fuzz x86_emulate() while we are at it
    ...
    memcpy(&_regs, ctxt->regs, sizeof(struct cpu_user_regs));
    memcpy(&_temp_regs, &_regs, sizeof(struct cpu_user_regs));
...
//printf("%llu eip\n", _regs.rip);
 ...
 done_prefixes:
    ...
    case 0x00 ... 0x05: add: /* add */
    	_temp_regs = _regs;
        emulate_2op_SrcV("add", src, dst, _temp_regs.eflags);
 ...
 cannot_emulate:
    printf("cannot emulate instruction...\n");
    return X86EMUL_UNHANDLEABLE;
}
